...
if "last_tested_key" not in st.session_state: st.session_state.last_tested_key = "" # Empêche de vider le quota avec des tests répétitifs
...
# ==========================================
# 2. GESTION DES CLÉS API ET QUOTAS (CORRIGÉ)
# ==========================================
...
if API_KEY:
    ...
    # On ne fait le test de quota (ping) QUE si la clé vient de changer
    if API_KEY != st.session_state.last_tested_key:
        ...
        # On mémorise la clé testée pour ne pas refaire le ping au prochain clic
        st.session_state.last_tested_key = API_KEY

    # Affichage dynamique des messages selon l'origine de la clé
    if is_user_key:
        if st.session_state.is_pro_active:
            st.sidebar.success("✅ **Clé personnelle validée**\n\n🚀 Vous avez des quotas PRO (Recherche Web active)")
        else:
            st.sidebar.warning("⚠️ **Clé personnelle validée**\n\n🐢 Quota PRO épuisé (Mode ÉCO actif)")
    else:
        if st.session_state.is_pro_active:
            st.sidebar.success("🌍 **Clé partagée active**\n\n🚀 Mode PRO disponible")
        else:
            st.sidebar.warning("🌍 **Clé partagée active**\n\n🐢 Quota partagé épuisé (Ajoutez votre clé pour passer en PRO)")
...
if mode == "Investissement Locatif":
    ...
    cash_flow = loyer_final - mensualite - (loyer_final * 0.2)
